...
def main():
    #Inputs
    current_year = 2023
    past_kr_csv = f'{current_year}_Keeper_Rounds.csv'
    ...
    draft = league.get_draft(1048428546022117377) #draft for UPCOMING YEAR (not league_id+1)
    draft_picks = league.get_draft_picks()
    prev_league = League(int(league._league['previous_league_id']))
    prev_rosters = prev_league.get_rosters()
    transactions = [league.get_transactions(i) for i in range(20) if league.get_transactions(i) != []]
    ...
    #This section creates the rosters for each manager immediately after the draft
    ...
    with open(past_kr_csv, 'r') as csvfile:
        ...
        for row_num, row in enumerate(file_reader):
            if row_num == 0 and "ï»¿" in row[0]: row[0] = row[0].replace('ï»¿', '')
            player_name = row[0]
            keeper_round = row[1]
            try: 
    ...
    krstr = f"{current_year+1}_keeper_round"
    for rost in rosters:
        rost['player_attr'] = {}
        for player in rost['players']:
            ...
            if    (rost['player_attr'][player]['was_dropped'] == True and rost['player_attr'][player]['on_same_roster'] == False)\
                ...
                rost['player_attr'][player][krstr] = f'{current_year+1} ADP Round + 4'
    ...
    with open(f'{current_year+1} Keeper Rounds.csv', 'w', newline='') as csvfile:
        ...
        for rost in rosters:
            ...
            for pid in rost['player_attr']: 
                dict_of_vals = {'Player Name': rost['player_attr'][pid]['full_name'],
                                ...
                                f'{current_year} keeper round': rost['player_attr'][pid]['prev_keeper_round'],
                                ...
                                f'{current_year+1} keeper round': rost['player_attr'][pid][krstr],
                                }
